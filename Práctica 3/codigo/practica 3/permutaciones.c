...
int aleat_num(int inf, int sup) {
    ...
    if (inf < 0 || sup < 0) return ERR; /*comprobamos que nos pasan correctamente los argumentos*/
...
int* genera_perm(int n) {
    ...
    if (n < 0) return NULL; /*Comprobamos que nos pasan bien el tamaño de la permutación a generar*/

    permutacion = (int*) malloc((n) * sizeof (int)); /*Reservamos memoria para la permutación*/
...
int** genera_permutaciones(int n_perms, int tamanio) {
    ...
    if (n_perms < 0 || tamanio < 0) return NULL; /*Comprobamos que nos pasan correctamente los argumentos de entrada*/

    permu = (int **) malloc(n_perms * sizeof (int*)); /*Reservamos la memoria para el puntero a permutaciones*/
    if(!permu) 	return NULL;	

    for (i = 0; i < n_perms; i++) {
        (permu)[i] = (genera_perm(tamanio));
        if((permu)[i]==NULL){ /*llamamos a genera_perm que nos generará permutaciones de tamaño tamanio*/	
free(permu);
        return NULL;
        }
    }
